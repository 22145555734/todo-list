...
- **合集与子集**：一个事项可加若干子集（如「408」包含 4 门课），子集可独立计时、不设等级；事项的总时长由各子集累加，自身不再单独计时。不加子集的事项行为不变。
- **学习统计**：柱状图展示每天 / 每周 / 每月各科目的独立时长与学习总时长，支持近 7 天 / 14 天 / 30 天 / 3 个月 / 一年及自定义日期范围；有子集时可在「合并子集 / 展开子集」间切换。
- **等级与称号**：每个事项独立升级，共 500 级、累计 500 小时，进度条实时显示到下一级还需多久（合集按各子集累加的总时长计算）。
